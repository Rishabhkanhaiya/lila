...
function loadConfig() {
  ...
  return { width: 380, height: 420, x: null, y: null, opacity: 0.98, ws_url: "ws://127.0.0.1:8765" };
}
...
function createWindow() {
  ...
  win = new BrowserWindow({
    ...
    resizable: false,
    hasShadow: true,
    focusable: true, // Must be true so buttons receive click and pointer events
    show: true,
    webPreferences: {
  ...
  let cursorInterval = setInterval(() => {
    ...
    try {
      ...
      // Normalized direction from Lila's center to user's mouse (-1.0 to 1.0)
  ...
  // IPC Handlers: only pass { forward: true } when ignore is TRUE
